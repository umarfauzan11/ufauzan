import { useState, useRef, useEffect, lazy, Suspense } from 'react'

const DeviceModel = lazy(() => import('./DeviceModel'))

export default function ProjectCard3D({ projects }) {
  const [activeIndex, setActiveIndex] = useState(0)
  const [transitionId, setTransitionId] = useState(0)
  const [isVisible, setIsVisible] = useState(false)
  const [shakeDir, setShakeDir] = useState(null)
  const sectionRef = useRef(null)

  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) setIsVisible(true)
      },
      { threshold: 0.2 }
    )
    if (sectionRef.current) observer.observe(sectionRef.current)
    return () => observer.disconnect()
  }, [])

  const nextProject = () => {
    setActiveIndex((prev) => (prev + 1) % projects.length)
    setShakeDir('next')
    setTransitionId((prev) => prev + 1)  // selalu naik, gak pernah sama
  }

  const prevProject = () => {
    setActiveIndex((prev) => (prev - 1 + projects.length) % projects.length)
    setShakeDir('prev')
    setTransitionId((prev) => prev + 1)
  }

  const currentProject = projects[activeIndex]

  return (
    <div className="project-showcase" ref={sectionRef}>
      <div className="project-showcase-viewport">
        {isVisible && (
          <Suspense fallback={
            <div className="project-card-3d-loading">
              <div className="loading-spinner small"></div>
            </div>
          }>
            <DeviceModel
              screenshotUrl={currentProject.image || '/img_web/placeholder-project.png'}
              shakeDir={shakeDir}
              transitionId={transitionId}
            />
          </Suspense>
        )}
      </div>

      <div className="project-showcase-info">
        <div className="project-showcase-nav">
          <button className="project-nav-btn" onClick={prevProject}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="15 18 9 12 15 6"></polyline>
            </svg>
          </button>

          <span className="project-showcase-counter">
            {String(activeIndex + 1).padStart(2, '0')} / {String(projects.length).padStart(2, '0')}
          </span>

          <button className="project-nav-btn" onClick={nextProject}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="9 18 15 12 9 6"></polyline>
            </svg>
          </button>
        </div>

        <h3 className="typography-h2 project-showcase-title">{currentProject.name}</h3>
        <p className="typography-body project-showcase-desc">{currentProject.description}</p>

        {currentProject.url && (
          <a href={currentProject.url} target="_blank" rel="noopener noreferrer" className="btn project-showcase-link">
            View Link
          </a>
        )}
      </div>
    </div>
  )
}
